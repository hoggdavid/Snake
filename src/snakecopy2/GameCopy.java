...
/* D E B U G (2/ randomize control)
		...
		if (x==(gen-1)){
			...
			best.score = individuals.get(0).score;
			for (int i=1;i<100;i++){
				if ((individuals.get(i).score)>(best.score)){
					best = individuals.get(i);
					best.score = individuals.get(i).score;
				}
			}
			ArrayList<Double>Numbers;
			Numbers = new ArrayList<Double>();
			Numbers.add((double)totalscore);
			Numbers.add((double)best.score);
			...
			try{
				GameCopy.write("gscr_scr", Numbers);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
